...
def help(args, error=None):
    ...
    stations.sort()
    # Make a string with , between each entry
    names = "  station => " + ", ".join(stations)
    # We want to split the string nicely
    splits = [0]
    comma = 0
    # We assume a terminal width of 80 so use 78 so that it is not at the very end
    WIDTH = 78
    # Keep finding a comma that will separate the string with as long a position as possible
    while comma + WIDTH < len(names):
        # The next comma should be at most WIDTH away
        comma = names.rfind(",", comma, comma + WIDTH)
        # Store the position of each comma
        splits.append(comma)
    # Add a , at the end
    splits.append(len(names))
    # Get the lines split at each split point
    lines = [names[start+2:end+1] for start, end in zip(splits, splits[1:])]

    print """Syntax: python %s station {date} time

date => {n day{s} from} day_specifier (where n is a positive integer)
day_specifier => days_of_the_week | tomorrow | now | today | next week
days_of_the_week => Monday | Tuesday ...| Sunday | Mon | Tue | ... | Sun
