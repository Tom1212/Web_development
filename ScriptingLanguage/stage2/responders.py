...
def details(weather):
	...
	:return: The web page without any results
	"""
	return """Weather for {location} at {time}</br>
	Temp: {temperature}<i class="wi wi-celsius"></i> Feels Like: {feelsLike}<i class="wi wi-celsius"></i><br/>
	Low: {low}<i class="wi wi-celsius"></i> High: {high}<i class="wi wi-celsius"></i><br/>
	Sunrise <i class="wi wi-sunrise"></i>: {sunrise} Sunset <i class="wi wi-sunset"></i>: {sunset}<br/>
	Wind: {windSpeed} kph from {windBearing} <i class="wi wi-wind.towards-{windDirection}-deg"></i><br/>
	Summary <i class="wi wi-{icon}"></i>: {summary}<br/>""".format(**weather)
...
def respondToSubmit(formData):
	...
	if "error" not in weather:
		...
		data += '<p class="bg-success lead">%s</p>' % details(weather)
	else:
